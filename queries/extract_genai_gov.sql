...
--
-- Strategy:
--   GenAI signal      → LOWER(AllNames) or LOWER(Quotations) matches at least
--                       one term from GENAI_LEXICON (proper nouns and model
--                       names appear in AllNames; contextual mentions appear
--                       in Quotations).
--   Governance signal → LOWER(Quotations) matches a GOV_LEXICON term, OR
--                       V2Themes contains a GOV_THEME_TAGS code, OR
--                       DocumentIdentifier (the article URL) contains a
--                       GOV_URL_SLUGS substring.
--
-- The theme-tag and URL-slug branches were ported from
-- Alt_impl/GDELT_Web_Sci.ipynb (cell 2) — see MERGE_PLAN.md for the full
-- rationale. Importantly, both signals are ANDed into the corpus definition
-- here (an article must be GenAI AND governance to be in scope, per
-- PLAN.md), not left as a derived flag the way Alt_impl's own query used it.
--
...
WHERE DATE(_PARTITIONTIME) >= '2022-11-01'
  ...
  -- Governance filter: GOV_LEXICON keyword in Quotations, OR a GOV_THEME_TAGS
  -- code in V2Themes, OR a GOV_URL_SLUGS substring in the article URL.
  AND (
